        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          <Surface className="p-6 sm:p-8">
            ...
            <div className="mt-8 border-t border-[var(--color-rule)]">
              {contact.methods.map((method, index) => (
                ...
                >
                  <div className="font-mono text-[0.68rem] tracking-widest text-[var(--color-muted)]">
                    {method.label}
                  </div>
          ...
          <Surface className="relative overflow-hidden p-6 sm:p-8">
            ...
            <form onSubmit={onSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                className="inline-flex items-center rounded-full border border-[var(--color-ink)] bg-[var(--color-ink)] px-5 py-3 font-mono text-[0.68rem] tracking-widest text-[var(--color-bg)] transition hover:-translate-y-0.5"
              >
                Send message →
              </button>
            </form>
...
export function Footer() {
  ...
  return (
    <footer className="mt-12 border-t border-[var(--color-rule)] py-8 sm:mt-16 sm:py-10">
      <Container className="flex flex-col gap-3 font-mono text-[0.68rem] tracking-widest text-[var(--color-muted)] sm:flex-row sm:items-center sm:justify-between">
        <span>
