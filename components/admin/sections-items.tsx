...
import { Field, Input, Textarea, Select, SectionCard, ItemRow, ThumbnailInput, Divider, moveItem } from './primitives'
import type { SiteContent, WorkItem, ExperienceItem, EducationItem, ContactMethod, WorkKind } from '@/lib/types'
...
export function WorkSection({ content, update }: { content: SiteContent; update: Updater }) {
  ...
  return (
    <SectionCard title="Selected Work" subtitle="Projects, publications, and ships" count={items.length}>
      {items.map((row, i) => (
        <ItemRow key={row.id || i} index={i} total={items.length}
          title={row.title} meta={[row.venue, row.year, row.kind].filter(Boolean).join(' · ')}
          onMoveUp={() => movItem(i, i - 1)} onMoveDown={() => movItem(i, i + 1)} onRemove={() => remItem(i)}>
          <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr 1fr 140px', gap: 12 }}>
            <Field label="ID" hint="3-digit"><Input value={row.displayId} onChange={e => updItem(i, { displayId: e.target.value })} /></Field>
            <Field label="Title" required><Input value={row.title} onChange={e => updItem(i, { title: e.target.value })} /></Field>
            <Field label="Venue / role"><Input value={row.venue} onChange={e => updItem(i, { venue: e.target.value })} /></Field>
            <Field label="Year"><Input value={row.year} onChange={e => updItem(i, { year: e.target.value })} /></Field>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: 12 }}>
            <Field label="Kind">
              <Select value={row.kind} onChange={v => updItem(i, { kind: v as WorkKind })}
      ...
      <button
        ...
        style={{
          ...
          background: 'var(--adm-card)', border: '1px solid var(--adm-rule)', color: 'var(--adm-ink)',
    ...
    <SectionCard title="Experience & Research" subtitle="Jobs, research roles, and publications" count={items.length}>
      {items.map((row, i) => (
        <ItemRow key={row.id || i} index={i} total={items.length}
          ...
          onMoveUp={() => movItem(i, i - 1)} onMoveDown={() => movItem(i, i + 1)} onRemove={() => remItem(i)}>
          <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: 12 }}>
            <Field label="Period" hint="e.g. 2025 — Now"><Input value={row.period} onChange={e => updItem(i, { period: e.target.value })} /></Field>
            <Field label="Role / title" required><Input value={row.role} onChange={e => updItem(i, { role: e.target.value })} /></Field>
          </div>
          <Field label="Organization / context"><Input value={row.org} onChange={e => updItem(i, { org: e.target.value })} /></Field>
          <Field label="Description"><Textarea rows={3} value={row.description} onChange={e => updItem(i, { description: e.target.value })} /></Field>
        </ItemRow>
      ))}
      <button
        onClick={() => update(c => ({ ...c, experience: [...c.experience, { id: `new-${Date.now()}`, period: '', role: '', org: '', description: '' }] }))}
        style={{ marginTop: 8, fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: 1, textTransform: 'uppercase', padding: '10px 14px', borderRadius: 3, cursor: 'pointer', background: 'var(--adm-card)', border: '1px solid var(--adm-rule)', color: 'var(--adm-ink)', transition: 'all 0.15s' }}>
        + Add experience
      </button>
    </SectionCard>
  )
      ...
      {items.map((row, i) => (
        <ItemRow key={row.id || i} index={i} total={items.length}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: 12 }}>
            <Field label="Year"><Input value={row.year} onChange={e => updItem(i, { year: e.target.value })} /></Field>
            <Field label="School" required><Input value={row.school} onChange={e => updItem(i, { school: e.target.value })} /></Field>
          </div>
          <Field label="Description"><Textarea rows={2} value={row.description} onChange={e => updItem(i, { description: e.target.value })} /></Field>
        </ItemRow>
      ))}
      <button
        onClick={() => update(c => ({ ...c, education: [...c.education, { id: `new-${Date.now()}`, year: '', school: '', description: '' }] }))}
        style={{ marginTop: 8, fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: 1, textTransform: 'uppercase', padding: '10px 14px', borderRadius: 3, cursor: 'pointer', background: 'var(--adm-card)', border: '1px solid var(--adm-rule)', color: 'var(--adm-ink)', transition: 'all 0.15s' }}>
        + Add school
      </button>
    </SectionCard>
...
export function ContactSection({ content, update }: { content: SiteContent; update: Updater }) {
  ...
  const updMethod = (i: number, patch: Partial<ContactMethod>) =>
    update(c => ({ ...c, contact: { ...c.contact, methods: c.contact.methods.map((m, j) => j === i ? { ...m, ...patch } : m) } }))
  ...
  return (
    <SectionCard title="Contact" subtitle="How people reach you" count={methods.length}>
      <Field label="Intro paragraph" hint="shown next to the form">
        <Textarea rows={2} value={content.contact.intro}
          onChange={e => update(c => ({ ...c, contact: { ...c.contact, intro: e.target.value } }))} />
      </Field>
      <Divider />
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--adm-muted)', letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 10 }}>Methods</div>
      {methods.map((row, i) => (
        <ItemRow key={row.id || i} index={i} total={methods.length}
          title={row.label} meta={row.value}
          onMoveUp={() => movMethod(i, i - 1)} onMoveDown={() => movMethod(i, i + 1)} onRemove={() => remMethod(i)}>
          <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr 1fr', gap: 12 }}>
            <Field label="Label" hint="Email, LinkedIn…"><Input value={row.label} onChange={e => updMethod(i, { label: e.target.value })} /></Field>
            <Field label="Display text"><Input value={row.value} onChange={e => updMethod(i, { value: e.target.value })} /></Field>
      ...
      <button
        onClick={() => update(c => ({ ...c, contact: { ...c.contact, methods: [...c.contact.methods, { id: `new-${Date.now()}`, label: '', value: '', href: '' }] } }))}
        style={{ marginTop: 8, fontFamily: 'var(--font-mono)', fontSize: 11, letterSpacing: 1, textTransform: 'uppercase', padding: '10px 14px', borderRadius: 3, cursor: 'pointer', background: 'var(--adm-card)', border: '1px solid var(--adm-rule)', color: 'var(--adm-ink)', transition: 'all 0.15s' }}>
        + Add method
      </button>
    </SectionCard>
